...
from detectron2.structures import clip_boxes
...
def fast_rcnn_inference(
    ...
    Returns:
        instances: (list): A list of N instances, one for each image in the batch,
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Dict[str, torch.Tensor]]):
        """
        Args:
            predictions: return values of :meth:`forward()`.
            proposals (list): proposals that match the features that were
                used to compute predictions. The ``proposal_boxes`` field is expected.

        Returns:
            list: same as `fast_rcnn_inference`.
    ...
    def predict_boxes(
            self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Dict[str, torch.Tensor]]
    ):
        """
        Args:
            predictions: return values of :meth:`forward()`.
    ...
    def predict_probs(
        ...
        num_inst_per_image = [len(p['proposal_boxes']) for p in proposals]
